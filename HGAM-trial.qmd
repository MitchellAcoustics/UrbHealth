...
DIAB_modG <- bam(DIABETES ~ s(est_vmiles, bs="tp") + s(est_ptrp, bs="tp") + s(ht_ami, bs="tp") + s(emp_gravity, bs="tp") + s(State, bs="re"), data=data, method="fREML", family="quasibinomial")
...
```{r}
pred_DiabG <- predict(DIAB_modG, data, type="response", se.fit=TRUE)

ggplot(data, aes(x=pred_DiabG$fit, y=DIABETES)) +
    geom_point() +
    geom_abline(intercept = 0, slope = 1) + 
    labs(x='Predicted Values', y='Actual Values')
